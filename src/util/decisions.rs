    ...
    pub fn new(response: &Value) -> Result<Self> {
        ...
        Ok(Self {
            winner: {
                ...
                let winner = get(&format!("https://statsapi.mlb.com/api/v1/people/{pitcher_id}?hydrate=stats(group=[pitching],type=[gameLog],gameType={game_type})"))?;
                let line = pitching_line(&winner, game_id)?;
            ...
            loser: {
                ...
                let loser = get(&format!("https://statsapi.mlb.com/api/v1/people/{pitcher_id}?hydrate=stats(group=[pitching],type=[gameLog],gameType={game_type})"))?;
                let line = pitching_line(&loser, game_id)?;
            ...
            save: {
                if let Some(pitcher_id) = response["liveData"]["decisions"]["save"]["id"].as_i64() {
                    let closer = get(&format!("https://statsapi.mlb.com/api/v1/people/{pitcher_id}?hydrate=stats(group=[pitching],type=[gameLog],gameType={game_type})"))?;
                    let line = pitching_line(&closer, game_id)?;
